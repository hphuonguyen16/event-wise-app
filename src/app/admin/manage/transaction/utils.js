...
export function applyFilter({
  ...
  if (filterData.name) {
    inputData = inputData.filter(
      (item) =>
        item.user.name.toLowerCase().includes(filterData.name.toLowerCase()) ||
  ...
  if (filterData.transaction_type) {
    if (filterData.transaction_type === "all") {
      inputData = inputData;
    } else {
      inputData = inputData.filter(
        (event) => event.transaction_type === filterData.transaction_type
      );
    }
  }
